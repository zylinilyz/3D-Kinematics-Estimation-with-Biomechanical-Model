...
class Test:
    ...
    def preprocess_frames(self, framefile, src_dir):
        ...
        if skip_frame is False:
            for i in range(self._opt.num_view):
                ...
                #print(str.split(names[frane_index], '_view')[0] + '_view_%s.png'%i)
    ...
    def set_intpu(self, input):
        self._input_image = input['input_image'][None].float().to(self.device)
    ...
    def _display_visualizer_test(self):
        ...
        for clip in self.video_list:
            ...
            if fps_out == 30:
                n_chunks = int(np.ceil(2 * n_frames / self._opt.chunk_length))  
            else:
                n_chunks = int(np.ceil(n_frames / self._opt.chunk_length))
            ...
            for ch_i in range(n_chunks):
                ...
                print(f"================ Chunk {ch_i} ====================")
                start_frame = 0 + ch_i * self._opt.chunk_length
                end_frame = start_frame + self._opt.chunk_length
                
                chunk_features = [None] * self._opt.chunk_length
                empty_frames = []
                for cnt, f_idx in enumerate(tqdm.tqdm(range(start_frame, end_frame))):
                    
                    if fps_out == 30:
                        if f_idx % 2 == 1:
                            empty_frames.append(min(n_frames-1, f_idx))
                            continue
                        
                        frame = frame_files[min(n_frames-1, f_idx // 2)]   
                    else:
                        frame = frame_files[min(n_frames-1, f_idx)]
                ...
                empty_filename = os.path.join(self.out_dir, f'ik_mot_files_{self.exp_name}', subject, action, f'interpolated_frames_{ch_i}.npy')        
                ...
                if skip_chunk is False:
                    # sequence prediciton
                    pred_feats = torch.concatenate(chunk_features, dim=0)
                    pred_feats = rearrange(pred_feats, '(b f) p c -> b f p c', b = 1)
            
                    # temporal prediction
                    pred = self._model._img_encoder.query_sequence(pred_feats)
                    
                    # B F 36 1, B F 22 3
                    pred_coords, pred_scales = self.reform_prediction(pred)
                    
                    pred_coords_tp = pred_coords[0].squeeze(-1).cpu().data.numpy()
                    pred_scales_tp = pred_scales[0].cpu().data.numpy()
                
                
                    sc_filename = os.path.join(self.out_dir, f'osim_model_{self.exp_name}', subject, action, f'osim_scales_{ch_i}_scaled.npy')
                    os.makedirs(os.path.dirname(sc_filename), exist_ok=True)
                    np.save(sc_filename, pred_scales_tp)
                
                
                    out_mot_file = os.path.join(self.out_dir, f'ik_mot_files_{self.exp_name}', subject, action, f'ik_{ch_i}_radians.mot')
                    os.makedirs(os.path.dirname(out_mot_file), exist_ok=True)
                    wrtieCoords2Mot(out_mot_file, pred_coords_tp, coord2idx, True)
                else:
                    print(f'Failed to detect any pose from {clip}, chunk:{ch_i}, frame:{start_frame}-{end_frame}')
